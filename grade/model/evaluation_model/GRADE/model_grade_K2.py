...
class GRADE(nn.Module):
    ...
    def __init__(self, args, model_config, data_config, embedding_init_value, device):
        ...
        self.linear_onehop_weight = MLPTransformConnector(
            linear_layer_dim = 300*(16+2),
            output_size = 300)
        self.linear_twohop_weight = MLPTransformConnector(
            linear_layer_dim = 300*(16+2),
            output_size = 300)
    ...
    def forward(self, mode, 
        ...
        score_of_pair_1 = self.get_score(pair_1_input_ids_raw_text, \
                                    ...
                                    pair_1_batch_onehop_embedding_matrix, \
                                    pair_1_batch_twohop_embedding_matrix, \
                                    pair_1_batched_adjs)
        if SCORES:
        ...
        score_of_pair_2 = self.get_score(pair_2_input_ids_raw_text, \
                                    ...
                                    pair_2_batch_onehop_embedding_matrix, \
                                    pair_2_batch_twohop_embedding_matrix, \
                                    pair_2_batched_adjs)
    ...
    def get_score(self, input_ids_raw_text, \
                        ...
                        batch_onehop_embedding_matrix, \
                        batch_twohop_embedding_matrix, \
                        batched_adjs):
        ...
        onehop_embedding = self.linear_onehop_weight(batch_onehop_embedding_matrix.reshape(batch_size, -1)).reshape(batch_size, -1, 300)
        twohop_embedding = self.linear_twohop_weight(batch_twohop_embedding_matrix.reshape(batch_size, -1)).reshape(batch_size, -1, 300)
        keyword_h_embed = keyword_h_embed + onehop_embedding + twohop_embedding 
